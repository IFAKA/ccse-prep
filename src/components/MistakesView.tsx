...
export default function MistakesView({ state }: { state: AppState; update: (state: AppState) => void }) {
  ...
  return <section className="view-enter">
    ...
    <div className="page-section flex flex-wrap gap-2">{["all", "weak", "mastered", "task1"].map((option) => <button key={option} onClick={() => setFilter(option)} className={`focus-ring min-h-11 rounded-full border px-4 py-2 text-xs font-semibold uppercase ${filter === option ? "border-[var(--label)] bg-[var(--label)] text-[var(--surface)]" : "border-[var(--separator)] text-[var(--secondary)]"}`}>{option}</button>)}</div>
    <div className="mt-6 grid gap-3 sm:mt-8">{missed.length ? missed.map((question) => <SurfaceCard key={question.id} className="p-4 sm:p-5"><div className="flex justify-between gap-4 text-xs text-[var(--secondary)]"><span>Task {question.task} · {question.id}</span><span>{state.questionStates[question.id]?.status}</span></div><h2 className="mt-3 text-lg font-semibold">{question.question}</h2><p className="mt-3 text-sm">Official: <b className="uppercase">{question.answer}</b> · {question.options[question.answer]}</p></SurfaceCard>) : <p className="rounded-2xl border border-dashed border-[var(--separator)] p-8 text-center text-[var(--secondary)]">No missed questions yet.</p>}</div>
  </section>;
}
