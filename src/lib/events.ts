...
export const blankState=():AppState=>({schemaVersion:CURRENT_SCHEMA_VERSION,questionStates:{},mockHistory:[],sessions:[],settings:{sound:false,dailyTarget:10,remindersEnabled:false,reminderHour:19,reminderMinute:0},memories:[],events:[]});
export function reduceEvent(state:AppState,event:AppEvent):AppState { if(state.events.some(e=>e.eventId===event.eventId)) return state; const next={...state,events:[...state.events,event]};
  ...
  if(event.type==="SETTINGS_UPDATED") next.settings={...state.settings,...event.payload as Partial<AppState["settings"]>};
  return next;
}
