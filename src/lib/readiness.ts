...
export function readiness(state:AppState, questions:readonly Question[]):Readiness {
  const seen=questions.filter(q=>state.questionStates[q.id]?.attempts>0).length; const recent=questions.flatMap(q=>{const s=state.questionStates[q.id];return s?.lastSeenAt?[{t:s.lastSeenAt,ok:s.correct>0&&s.incorrect===0}]:[]}).sort((a,b)=>b.t-a.t).slice(0,300); const accuracy=recent.length?recent.filter(x=>x.ok).length/recent.length:0;
  ...
  const mocks=state.mockHistory.slice(0,10); const mockMet=mocks.length>=10&&mocks.every(m=>m.score>=20);
