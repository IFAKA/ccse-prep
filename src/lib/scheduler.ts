...
export function recordAnswer(previous:QuestionState|undefined, isCorrect:boolean, now=Date.now(), responseMs=0):QuestionState {
  ...
  const status = !isCorrect ? "weak" : consecutive >= 3 ? "mastered" : "learning";
  const gap = !isCorrect ? 5 : consecutive===1 ? 0 : consecutive===2 ? 1 : consecutive===3 ? 3 : 7;
  return {...s, attempts, correct, incorrect, consecutiveCorrect:consecutive, firstSeenAt:s.firstSeenAt??now, lastSeenAt:now, lastResponseMs:responseMs, nextReviewAt:now+(gap*24*60*60*1000), status};
}
